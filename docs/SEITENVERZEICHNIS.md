...
| Datenschutz | [Link](https://www.sanktbonifatius.de/datenschutz/) | `src/pages/datenschutz.astro` | Werner | ✅ erledigt | Page-ID 49118, eigene Datenschutz*-Komponenten (Präfix `astro-dz`): Hero (Verlauf), Body, CTA → Pfarrbüro. **Gezielt für das Astro-/Netlify-Setup angepasst** (nicht 1:1 aus WP): Hosting = Netlify; entfernt Google Analytics, Borlabs, reCAPTCHA, DigiAccess, domainfactory, Zahlungsverkehr (im Astro-Setup nicht vorhanden); aufgenommen ChurchDesk-Embeds (Formulare/Spenden/Kalender), YouTube/Vimeo, Bunny Fonts. Playfair Display zugleich von Google Fonts auf **Bunny Fonts** umgestellt (Base.astro) → komplett google-frei. E-Mails auf `@sanktbonifatius.de` korrigiert. **Auf KDG umgestellt** (Kirchliches Datenschutzgesetz statt DSGVO): Rechtsgrundlagen §§ 6 KDG (lit. b Einwilligung / c Vertrag / d Pflicht / f kirchl. Aufgabe; lit. g „berechtigtes Interesse" gilt für öffentl.-rechtl. kirchl. Stellen NICHT → lit. f), Betroffenenrechte §§ 17–23, Drittland § 40, Beschwerde § 48 KDG; zuständige Aufsicht = **Katholisches Datenschutzzentrum Frankfurt (KDSZ)**, Roßmarkt 23, 60311 Frankfurt. **Interim-Entwurf** — muss vom Pfarrei-DSB (Martin Kestler) / Bistum Limburg (Betriebl. DSB Kirchengemeinden, Michael Hilpüsch) freigegeben werden; rechtsverbindliche Endfassung folgt. ⚠️ vor Go-Live prüfen: lit. f/g je Verarbeitung, Netlify-Firmenanschrift, ChurchDesk-Datenschutz-URL, AVV mit Netlify/ChurchDesk |
